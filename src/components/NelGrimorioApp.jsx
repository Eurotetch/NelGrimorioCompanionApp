...
const NelGrimorioApp = () => {
  ...
  const menu = [
    ...
    { icon: Gamepad2, label: 'Lista Giochi', path: '/games' },
    { icon: MessageCircle, label: 'Consigliami tu!', path: '/recommend' },
    { icon: ShoppingCart, label: 'Mercatino Compravendite', path: '/marketplace' },
    { icon: Calendar, label: 'Programma Eventi', path: '/events' },
    { icon: Send, label: 'Invia Feedback', path: '/feedback' }
  ];
  ...
  return (
    <div className="min-h-screen bg-stone-900 text-white overflow-x-hidden" style={{ fontFamily: 'var(--font-main, system-ui, -apple-system, sans-serif)' }}>
        ...
        <div className="p-4">
          ...
          {menu.map((item, i) => (
			  <button 
				...
				className="w-full flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-stone-800 text-left mb-2"
			  >
            ...
            <div className="flex items-center gap-2">
              ...
              {isAuth ? (
                <button 
				  ...
				  className="hidden sm:flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-lg font-semibold text-stone-900 text-sm hover:from-yellow-400 hover:to-yellow-500 transition-colors cursor-pointer"
				>
                  <User className="w-4 h-4" />
                  <span className="hidden md:inline">Francesco</span>
                </button>
              ) : (
                <button onClick={() => navigate('/login')} className="flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r from-cyan-500 to-cyan-600 rounded-lg font-semibold text-xs sm:text-sm">
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 py-4 sm:py-8">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
            {games.map(game => (
              <div key={game.id} onClick={() => !isAuth ? navigate('/login', { state: { from: '/' } }) : setShowDateModal(true)} className="bg-stone-800 rounded-xl overflow-hidden hover:scale-105 transition-transform cursor-pointer">
      ...
      {/* Auth Modal NON UTILIZZATA: Usa pagina /login */}
		  {
		// {showAuth && (
      ...
      {showGameDetail && (
        <div className="fixed inset-0 bg-black bg-opacity-80 z-50 flex items-center justify-center p-4">
          <div className="bg-stone-800 rounded-2xl max-w-4xl w-full p-6 sm:p-8 border-2 border-yellow-500 max-h-screen overflow-y-auto">
            ...
            <button onClick={() => { setShowGameDetail(null); !isAuth ? navigate('/login', { state: { from: '/' } }) : setShowDateModal(true); }} className="w-full bg-gradient-to-r from-yellow-500 to-yellow-600 py-3 sm:py-4 rounded-lg font-bold text-stone-900 text-sm sm:text-base">
              🎲 Voglio Giocarci!
            </button>
          </div>
        </div>
